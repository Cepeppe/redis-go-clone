...
// An entry is: key_len(uint_32) key(string) data_len(uint_32)
// returns key and value and error
func read_entry(f *os.File) (string, string, error) {
	...
	return string(key_buf), string(data_buf), err
}

// An entry is: key_len(uint_32) key(string) data_len(uint_32) data(string)
// returns key and value and error
